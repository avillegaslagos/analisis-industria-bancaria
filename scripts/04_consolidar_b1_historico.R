...
# ----------------------------------------------------------
# Rutas (ABSOLUTAS – SIN AMBIGÜEDAD)
# ----------------------------------------------------------
ruta_b1 <- "C:/Users/avill/Desktop/Diplomado R/temp_cmf/b1"

ruta_output <- "C:/Users/avill/Desktop/Diplomado R/temp_cmf/processed"
...
leer_b1 <- function(path) {
  ...
  if (length(lineas) < 2) return(NULL)
  
  df_raw <- read_table(
    paste(lineas[-1], collapse = "\n"),
    col_names = c("ifrs", "clp", "uf", "mx", "total"),
    col_types = cols(.default = col_character())
  )
  
  df_raw %>%
    filter(ifrs %in% ifrs_objetivo) %>%
    mutate(
      banco_codigo = banco_codigo,
      periodo = as.Date(paste0(periodo_yyyymm, "01"), "%Y%m%d"),
      
      # Conversión correcta CMF (coma decimal)
